...
$result = $conn->query("SELECT * FROM MOVIES WHERE rating > 8 ORDER BY rating DESC LIMIT 10");
...
<html lang="en">
  <head>
    ...
    <meta name="description" content="Top rated movies on ReelFlix — films with a rating above 8/10." />
    <title>Top Rated Movies - ReelFlix</title>
    <link rel="icon" type="image/x-icon" href="images/toprated.webp" />
  ...
  <body>
    ...
    <header class="header reveal">
      <h1><b>TOP RATED</b></h1>
      <p style="color:var(--muted);margin-top:6px;">The top 10 highest-rated films in our collection</p>
    </header>
      ...
      <div id="movieList" class="movie-grid">
        <?php if (count($movies) > 0): ?>
          <?php foreach ($movies as $i => $movie): ?>
            ...
            <div class="movie-card reveal" onclick="window.location.href='moviedetails.php?id=<?php echo $movie['ID']; ?>'">
              ...
              <h3><?php echo ($i + 1) . '. ' . htmlspecialchars($movie['TITLE']); ?></h3>
        ...
        <?php else: ?>
          <div class="empty-state">
            ...
            <h3>No top-rated movies yet</h3>
            <p>Check back as more movies are added.</p>
          </div>
        <?php endif; ?>
      </div>
    </main>
